...
interface PrintableIncomeByCustomerProps {
    id: string;
...
const PrintableIncomeByCustomer: React.FC<PrintableIncomeByCustomerProps> = ({ id, reportData, settings, startDate, endDate }) => {
    ...
    return (
        <div id={id} className="bg-white p-8 w-[210mm] min-h-[297mm] text-xs">
            <header className="flex justify-between items-start mb-8 border-b pb-4">
                ...
                <h1 className="text-3xl font-bold text-slate-400 uppercase tracking-widest">Income By Customer</h1>
            </header>
                ...
                <table className="w-full text-left table-fixed text-sm">
                    <thead>
                        <tr className="bg-slate-100 text-slate-600">
                            <th className="p-2 font-semibold w-1/2">Customer Name</th>
                            <th className="p-2 font-semibold text-right w-1/4">Total Billed</th>
                            <th className="p-2 font-semibold text-right w-1/4">Outstanding Balance</th>
                        </tr>
                    </thead>
                    <tbody>
                        {reportData.map(row => (
                            <tr key={row.id} className="border-b">
                                ...
                                <td className="p-2 text-right font-medium text-red-600">{formatCurrency(row.outstandingBalance)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
export default PrintableIncomeByCustomer;